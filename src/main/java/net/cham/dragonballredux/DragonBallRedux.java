package net.cham.dragonballredux;
...
@Mod(DragonBallRedux.MOD_ID)
public class DragonBallRedux {
    // Define mod id in a common place for everything to reference
    public static final String MOD_ID = "dragonballredux";
    // Directly reference a slf4j logger
    public static final Logger LOGGER = LogUtils.getLogger();

    public DragonBallRedux(FMLJavaModLoadingContext context) {
    ...
    private void addCreative(BuildCreativeModeTabContentsEvent event) {
        if(event.getTabKey() == CreativeModeTabs.FOOD_AND_DRINKS){
            //event.accept(ModItems.*insert item name*);
        }

        if(event.getTabKey() == CreativeModeTabs.BUILDING_BLOCKS){
            //event.accept(ModBlocks.*insert block name*);
        }
    }
